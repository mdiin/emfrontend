...
| File | Contents |
|------|----------|
| `2026-09-17-grey-box.md` | The grey-box session: the actual cause (a placement naming an element record the client was never given — the snapshot reached elements only where the projection placed or wired them, and `PlaceElement` emits only the placement), the fix in `../thecli` (the snapshot's required `elements` registry built through one `canonical-entity`, with the docs, schema, spec and drift check around it) plus the frontend guard (`store/element-loaded?`, `canvas/card-presentation`, the `surface Canvas` guarantee `UnloadedElementShownAsPlaceholder`, and the ElementDetails three-way completeness reading that stopped the panel claiming "information complete" for a record it did not hold), the spec-step element-ref unification, `118: All tests passed!` here and `210 tests … 0 failures` in `../thecli`, four weed passes per repo ending CLEAN, and the recorded disproof of the reload note's cause 2. |
